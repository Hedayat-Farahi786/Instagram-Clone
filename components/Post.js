...
function Post({username, userImg, img, caption, verified}) {
  return (
    <div className="bg-white my-7 border rounded-sm">
        {/* Header */}
        <div className="flex items-center p-5">
            <img src={userImg} className="rounded-full h-12 w-12 object-contain border p-1 mr-3" alt="" />
            <p className="flex-1 font-bold flex items-center">{username} {verified ? <BadgeCheckIcon className="pt-0.5 h-5 ml-1 text-blue-500"/> : ""}</p>
